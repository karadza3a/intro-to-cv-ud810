...
def hough_peaks(hough_acc, max_num_peaks, threshold=None, nhood=None, verbose=False, include_acc=False):
    h = hough_acc.copy()
    ...
    for i in range(max_num_peaks):
        ...
        acc = h[rho, theta]
        if verbose:
            print("%5d %5d %5.3f" % (rho, theta, acc))
        if acc < threshold:
            break
